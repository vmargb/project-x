...
#+begin_src emacs-lisp
  (use-package project-x
    ...
    (setq project-prompter #'project-x--project-prompt) ; use our prompter (optional)
    (project-x-mode 1))
#+end_src
...
| =C-x p d=    | =project-x-delete-session=  | Delete session data |

**** Workflow
To add a new project(+ root marker) do =C-x p a= with DIR, and to remove it from the project list do =project-forget-project=. Enter the project with =project-switch-project= or =C-x p j=, save the current session state with =C-x p w=.

If you make any changes(buffers, windows etc.) =C-x p J= will revert back to the /last/ saved session state(for the project you are currently in). And optionally use =C-x p r= to rename the project in the list to whatever you want, which will only change the name of the /session/ without touching the actual project name or its path.

To close the session simply use project.el's =project-kill-buffers=. You can go back to your previous window configuration with =winner-undo=, or even better: use Emacs' =tab-bar-mode= or something like Tabspaces/otpp.el.
...
** Changelog
- /New in v0.2.2:/ Fixed a subtle but *major* bug where =squatter-buffers= prevent =project-switch-project= from going through.
