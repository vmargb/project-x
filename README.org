...
Starting with Emacs 29, you can install packages directly from version control using =use-package= and the =:vc= keyword.
...
5. To close the session, simply use project.el's =project-kill-buffers=. You can go back to your previous window configuration with =winner-undo=, or use Emacs' =tab-bar-mode= (or packages like Tabspaces/otpp.el).
...
** Changelog
- /New in v0.2.2:/ Fixed a subtle but *major* bug where Emacs's =uniquify= squatter buffers prevent =project-switch-project= from correctly restoring sessions.
