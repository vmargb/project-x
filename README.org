...
- The only state saved is your project files, =dired= buffers, =magit-status= buffers, =eshell= buffers, =compilation= buffers, and the current frame configuration. No minor modes, registers, or other special buffers (help buffers, scratch, etc.) are recorded. For complete recall, look into the built-in Desktop library.
