...
Existing sessions saved /before/ this feature keep working as-is: they're just treated as the ~"default"~ layout the first time they're loaded.
...
** Changelog
- /New in v0.3.1:/ Multiple named window layouts per project (=project-x-window-state-save-as=, =project-x-switch-layout=, =project-x-delete-layout=, bound under =C-x p l=).
