...
- =project-kill-buffers= closes the project & the session, optionally saving the session state right where you left off
- =project-forget-project= now automatically clears session data on top of removing the project
...
- =project-x-save-extra-buffers=: Boolean to control whether special non-file buffers (=magit-status=, =eshell=, =compilation=) are tracked. Defaults to =nil=. If toggled to =t=, these buffers will not be skipped during save/restore. If switched back to =nil=, existing extra buffers found in saved states will be safely ignored upon restoration.
...
4. If you make any changes (buffers, windows, etc.), =C-x p J= will revert back to the /last/ saved session state (for the project you are currently in).
5. Optionally use =C-x p r= to rename the project in the list. This only changes the name of the /session/ without touching the actual project name or its path.
6. To close the session, simply use project.el's =project-kill-buffers=.
...
- The only state saved is your project files, =dired= buffers, and (if enabled via =project-x-save-extra-buffers=) =magit-status=, =eshell=, and =compilation= buffers. No minor modes, registers, or other special buffers (help buffers, scratch, etc.) are recorded. For complete recall, look into the built-in Desktop library.
