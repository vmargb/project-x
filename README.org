...
**** Workflow
After entering a project with =project-switch-project= or =C-x p j=, save the current session state with =C-x p w=, if you make any changes(buffer and window configuration) =C-x p J= will revert back to the /last/ saved session state. To close the session do =project-kill-buffers=, you can load it back at any time which will restore the session. If you want to add a new project(with root marker) do =C-x p a=, and =project-forget-project= to remove it. Lastly to rename a project do =C-x p r= which will only change the name of the /session/ without changing the actual project name or its path.
