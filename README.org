...
Having to learn two different sets of commands.

The traditional Emacs workflow is to use project.el or projectile for *project management*, while using *session management* packages to handle the creation of "sessions" and restoring that session the way you left it(which may include a project). Now these packages do offer more than what I just described, but in many peoples workflows, they are mostly used for this basic separation of concerns.

Project-x collapses that split: every project is treated as a session, and restoring a session is identical to restoring the project. That means you keep the exact =project.el= keybindings you already know, while Project-x transparently handles saving, restoring, and syncing the project’s session state (buffers, windows, point, etc.). So:
- =project-switch-project= now opens/changes sessions too
- =project-kill-buffers= closes & optionally saves the session where you left it
