...
The traditional Emacs workflow is to use =project.el= or =projectile= for *project-management*, and a different package for *session-management*, which saves and restores your workspace. While these packages can do much more, many use them only to save and restore a project as they left it. That split forces you to remember two different groups of commands.

=Project-x= collapses that split: every project is a session, and restoring a session is identical to restoring a project. You keep the same =project.el= keybindings you're already familiar with, while =Project-x= transparently handles saving, restoring, and syncing the project’s session state.
...
You can install =project-x= from [[https://melpa.org/#/project-x][MELPA]]:
...
There are five main customization options:
...
- =project-x-restore-last-project-on-startup=: Boolean controlling whether Emacs will automatically open and switch to the project you were last working on when you exited. Defaults to =nil=.
...
- Terminal buffers are restored without their scroll history and state. At the moment, the only way to solve this is to use =tmux=.
...
**** ..Projectile?
=Projectile= now also supports project [[https://docs.projectile.mx/projectile/sessions.html][sessions]]. The main difference is that the =project.el= workflow is much lighter and already built-in, so existing commands you already know are session-aware, whereas Projectile provides its own framework with its own command set. Projectile offers a lot more features beyond what project.el can do, but if all you're looking for is *session persistence*, then you don't have to switch to an entirely new framework, project-x is just a small addition to what you already have.
...
** Changelog
- /New in v0.3.2:/ Added =project-x-restore-last-project= to optionally continue from the last active project where you left off. Added extra terminal support and you can now cycle/next/previous through project layouts.
...
- /New in v0.2.4:/ =project-x-save-extra-buffers= to let users opt-out of saving or restoring special buffers (=magit=, =eshell=, =compilation=). Added general optimisations and fixed an error caused by projects on an unreachable host.
